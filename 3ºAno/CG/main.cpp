...
void computeNormal(int i, int j, std::vector<float>& normal) {
	...
	// Cálculo do numerador da expressão da normal

	float resNumerador[3];

	cross(v1,v2,resNumerador);

	// Cálculo do denominador da expressão da normal

	float resDenominador[3];

	cross(v1,v2,resDenominador);

	normalize(resDenominador);
	...
	for (int i = 0; i < 3; i++){
		normalCalc[i] = resNumerador[i] / resDenominador[i];
	}

	for (int i = 0; i < 3; i++){
		normal.push_back(normalCalc[i]);
	}
