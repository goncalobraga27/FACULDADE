...
pred reserve[u : User, r : Resource] {
	...
	all us : User - u | u.reservations' = u.reservation 
...
pred cancel[u : User, r : Resource] {
	...
	all us : User - u | u.reservations' = u.reservations 
...
pred cleanup {
	...
	all r : Resource | (r not in Available and r not in User.reservations) implies Available' = Available + r 
...
run Scenario3 {
	...
	some u : User , some disj r1,r2 : Resource {reserve[u,r1];reserve[u,r2];cancel[u,r1];use[u,r2];cleanup}
} expect 1
